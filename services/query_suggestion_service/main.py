...
def load_user_queries(dataset: str, user_id: Optional[str] = None) -> tuple:
    """Load user queries for a given dataset from the TSV file (with counts). If user_id is provided, filter by user."""
    if not os.path.exists(USER_QUERIES_FILE):
        return [], {}
    queries = []
    counts = {}
    with open(USER_QUERIES_FILE, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) >= 2 and parts[0] == dataset:
                q = parts[1].strip()
                uid = parts[3] if len(parts) > 3 else None
                if user_id is None or (uid and uid == user_id):
                    queries.append(q)
...
class SmartQuerySuggestionService:
    ...
    def get_trending_suggestions(self, dataset: str, top_k: int) -> List[Dict]:
        """Get trending queries based on recent time window (default: last 7 days)"""
        days_window = 7  # عدد الأيام لتحليل الترند
        now = datetime.utcnow()
        queries = []
        timestamps = []
        # تحميل الاستعلامات مع التواريخ
        if not os.path.exists(USER_QUERIES_FILE):
            return []
        with open(USER_QUERIES_FILE, "r", encoding="utf-8") as f:
            for line in f:
        ...
        # تصفية الاستعلامات حسب الفترة الزمنية
        recent_queries = [q for q, ts in zip(queries, timestamps) if (now - ts).days < days_window]
        ...
        trending_keywords = [word for word, freq in keyword_freq.most_common(20)]
        # إنشاء اقتراحات الترند
        trending_queries = []
...
def get_stats():
    ...
    for dataset in ["argsme", "wikir"]:
        queries = suggestion_service.load_queries(dataset)
        stats[dataset] = {
            "total_queries": len(queries),
            "original_queries": len([q for q in queries if q['type'] == 'original']),
            "enhanced_queries": len([q for q in queries if q['type'] == 'enhanced']),
            "popularity_scores_calculated": bool(suggestion_service.popularity_scores.get(dataset)),
            "similarity_matrix_built": bool(suggestion_service.similarity_matrix.get(dataset))
        }
    return stats
