...
class QueryRequest(BaseModel):
    ...
    top_k: Optional[int] = None  # For API Gateway compatibility
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "/predict": "POST - Predict topics (alias for detect-topics)",
            "/topics": "GET - Get topics for dataset",
...
async def detect_topics(request: QueryRequest):
    ...
    try:
        ...
        input_text = request.query if request.query else request.text
        if not input_text or not input_text.strip():
            raise HTTPException(status_code=400, detail="Query or text cannot be empty")
...
@app.post("/predict")
async def predict_topics(request: QueryRequest):
    """Predict topics for a given text (alias for detect-topics)"""
    try:
        # Get the actual text to process
        input_text = request.query if request.query else request.text
        if not input_text or not input_text.strip():
        ...
        # Return simplified format for API Gateway compatibility
        return {
            "topics": [
                {
                    "topic": topic['topic'],
        ...
        logger.error(f"Error in predict_topics: {e}")
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")

@app.get("/topics")
async def get_topics(dataset: str = Query("argsme", description="Dataset to use: 'argsme' or 'wikir'"), limit: int = 50):
    """Get topics for a specific dataset"""
    try:
        topic_model = get_model(dataset)
        if not topic_model:
            raise HTTPException(status_code=500, detail=f"{dataset.upper()} topic model not loaded")
        ...
        for topic_data in global_topics[:limit]:
            if len(topic_data) >= 3:
                ...
                topics_list.append({
                    ...
                    'frequency': count,
                    'coverage_ratio': ratio,
                    'doc_count': topic_data[3].get('doc_count', 0) if len(topic_data) > 3 else 0
                })
